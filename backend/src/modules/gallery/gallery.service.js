...
const mapItem = item => ({ id: item.id, sportId: item.sport_id, mediaType: item.media_type, storagePath: item.storage_path, publicUrl: `${env.supabaseUrl}/storage/v1/object/public/event-media/${item.storage_path}`, titleId: item.title_id, titleEn: item.title_en, altId: item.alt_id, altEn: item.alt_en, sortOrder: item.sort_order, status: item.status, publishedAt: item.published_at, createdAt: item.created_at, updatedAt: item.updated_at });
